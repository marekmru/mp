      ...
      <v-form ref="form" @submit.prevent="submitForm" v-model="isFormValid" validate-on="input"
        ...
        <WithFormDefaults>
          <v-card-text class="pa-0">
            ...
            <template v-if="isFirstProjectMode">
...
const dialogTitle = computed(() => {
  ...
  if (isFirstProjectMode.value) return 'Add First Mediaplan';
...
watch(() => props.modelValue, (isVisible, wasVisible) => {
  try {
    ...
    if (isVisible && !wasVisible) {
      if (props.mode === 'edit-project') {
        if (props.projectData) {
          ...
          // Edit mode, but no projectData was provided
          resetFormFields();
        }
      } else { // Create modes (any mode that is not 'edit-project')
        resetFormFields();
      }
    }
    // --- Dialog is CLOSING ---
    ...
    console.error("Error occurred inside modelValue watcher:", error);
  }
});
